...
UserInputService.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseWheel then
        camZoom = math.clamp(camZoom - input.Position.Z * 5, 5, 150)
    end
    if camDragging and input.UserInputType == Enum.UserInputType.MouseMovement then
        if lastMousePos then
            local delta = input.Position - lastMousePos
            camYaw   = camYaw   - delta.X * 0.005
            camPitch = math.clamp(camPitch + delta.Y * 0.005, math.rad(-80), math.rad(80))
        end
        lastMousePos = input.Position
    end
end)

-- Right-click drag = หมุน
...
RunService:BindToRenderStep("FinderCamLock", Enum.RenderPriority.Camera.Value + 1, function()
    ...
    for key, data in pairs(modeData) do
        if data.cameraLocked then
            ...
            if hrp then
                local cx = hrp.Position.X + camZoom * math.sin(camYaw) * math.cos(camPitch)
                local cy = hrp.Position.Y + camZoom * math.sin(camPitch)
                local cz = hrp.Position.Z + camZoom * math.cos(camYaw) * math.cos(camPitch)
                Camera.CFrame = CFrame.new(Vector3.new(cx, cy, cz), hrp.Position)
            end
